...
bill_name <- df_bills_on_topic[df_bills_on_topic$short_title == "FAA Reauthorization Act of 2018", "short_title"]
bill_long_title <- df_bills_on_topic[df_bills_on_topic$short_title == "FAA Reauthorization Act of 2018", "title"]
sponsor <- df_bills_on_topic[df_bills_on_topic$short_title == "FAA Reauthorization Act of 2018", "sponsor_name"]
sponsor_party <- df_bills_on_topic[df_bills_on_topic$short_title == "FAA Reauthorization Act of 2018", "sponsor_party"]
sponsor_state <- df_bills_on_topic[df_bills_on_topic$short_title == "FAA Reauthorization Act of 2018", "sponsor_state"]
cosponsors <- df_bills_on_topic[df_bills_on_topic$short_title == "FAA Reauthorization Act of 2018", "cosponsors"]
latest_major_action <- df_bills_on_topic[df_bills_on_topic$short_title == "FAA Reauthorization Act of 2018", "latest_major_action"]
link_to_text <- df_bills_on_topic[df_bills_on_topic$short_title == "FAA Reauthorization Act of 2018", "congressdotgov_url"]
passed_house <- df_bills_on_topic[df_bills_on_topic$short_title == "FAA Reauthorization Act of 2018", "house_passage"]
```
...
> Part one is a table of recent `r topic` related bills. Part two is information on the `r bill_name`. Part three is information about my local representative, `r name`. 

> The Legislative Branch, also called Congress, is made up of a group of people called legislators. The US has a bicameral legislature (meaning two houses of Congress). They are the Senate, whose members are called senators, and the House of Representatives, whose members are called representatives or congressmen. Senators are elected for a six year term while representatives are elected for two year terms. 
...
- **latest_major_action:** If a bill introduced in one house passes in another house, the latest major action should read "received in the...[other house that the bill was not introduced in]" If a bill is passed by both houses, then the latest major action should read "Presented to President." If a bill becomes a law, the latest major action should read "Became Public Law No: 115-xxx" with 115 meaning the 115th Congress.
...
> The `r bill_name` should become law. It has bipartisan support, provides funding for critical infrastructure, and addresses recent problems in the airline industry such as passenger bumping, increase in pets being brought on-board, and loss of planes in the middle of the ocean.


#### Bill Actions
On `r passed_house`, the `r bill_name` passed the House. It will now go to the Senate to be voted on.
